...
window.addEventListener('scroll', () => {
    ...
    if (aboutImageParallax && window.innerWidth > 992 && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        ...
        // --- Service suggestion popover ---
        (function() {
            const suggestionsMap = {
                'Life Insurance': [
                    { label: 'TATA AIA - Term + ULIP combo', badge: 'Best', url: 'https://www.tataaia.com' },
                    { label: 'LIC - Endowment plans', badge: 'Trusted', url: 'https://www.licindia.in' },
                    { label: 'Custom Family Protection Plan', badge: 'Recommended' }
                ],
                'Health Insurance': [
                    { label: 'Star Health - Comprehensive', badge: 'Best', url: 'https://www.starhealth.in' },
                    { label: 'TATA AIG - Senior & Family Floater', badge: 'Good', url: 'https://www.tataaig.com' },
                    { label: 'Care - Budget Friendly', badge: 'Value', url: 'https://www.careinsurance.com' }
                ],
                'Car Insurance': [
                    { label: 'TATA AIG - Comprehensive', badge: 'Best', url: 'https://www.tataaig.com' },
                    { label: 'ICICI Lombard - Zero Dep', badge: 'Popular', url: 'https://www.icicilombard.com' },
                    { label: 'Liberty - Affordable', badge: 'Value', url: 'https://www.libertyinsurance.in' }
                ],
                'General Insurance': [
                    { label: 'TATA AIG - Travel & Asset Cover', badge: 'Best', url: 'https://www.tataaig.com' },
                    { label: 'Custom Asset Protection', badge: 'Recommended' }
                ],
                'Mutual Funds & Investments': [
                    { label: 'Prudent - Long term growth', badge: 'Best', url: 'https://www.prudentcorporate.com' },
                    { label: 'FundzBazaar - SIP starter', badge: 'Good', url: 'https://www.fundzbazaar.com' }
                ],
                'Payments Bank Services': [
                    { label: 'Airtel Payments Bank - Digital accounts', badge: 'Best', url: 'https://www.airtel.in/bank' }
                ]
            };
            ...
            popover.innerHTML = `
                <div class="popover-header">Suggested options</div>
                <ul class="suggestion-list" aria-live="polite"></ul>
            `;
            ...
            function openPopoverForCard(card) {
                const titleEl = card.querySelector('.card-content h3');
                const serviceName = titleEl ? titleEl.textContent.trim() : 'Service';
                const suggestions = suggestionsMap[serviceName] || [ { label: 'Contact for personalized advice', badge: 'Talk' } ];

                const listEl = popover.querySelector('.suggestion-list');
                listEl.innerHTML = '';
                suggestions.forEach((s, idx) => {
                    const li = document.createElement('li');
                    li.className = 'suggestion-item' + (idx === 0 ? ' best' : '');
                    li.tabIndex = 0;
                    // build inner content; include open-link button when url is provided
                    const safeLabel = s.label;
                    li.innerHTML = `<div class="badge">${s.badge || ''}</div><div class="text">${safeLabel}</div>`;
                    if (s.url) {
                        const openBtn = document.createElement('button');
                        openBtn.className = 'open-link';
                        openBtn.title = 'Open provider site';
                        openBtn.setAttribute('aria-label', `Open ${safeLabel} website`);
                    ...
                    li.addEventListener('click', (ev) => {
                        // If this is the 'best' suggestion (first item) and has a URL, open it
                        if (idx === 0 && s.url) {
                            ev.stopPropagation();
                            window.open(s.url, '_blank', 'noopener');
                            return;
                        }
                        onSelectSuggestion(serviceName, s);
                    });
                ...
                const popW = 320; // estimated
                const gap = 12;
                let left = rect.right + gap;
                let top = rect.top + window.scrollY;

                // if not enough space on right, place below
            ...
            function onSelectSuggestion(serviceName, suggestion) {
                // Prefill contact form message and scroll to contact
                const messageField = document.getElementById('message');
                if (messageField) {
                    messageField.value = `Hello, I'm interested in "${serviceName}" — specifically: ${suggestion.label}. Please advise best next steps.`;
                    messageField.focus();
                }
                ...
                if (contactSection) {
                    ...
                    window.scrollTo({ top: contactSection.offsetTop - offset, behavior: behavior });
                }
                closePopover();
            }
            ...
            document.addEventListener('click', (e) => {
                ...
                if (card) {
                    ...
                    if (activeCard === card && popover.classList.contains('visible')) {
                        ...
                        openPopoverForCard(card);
                    }
                    return;
                }
                // click outside -> close
                if (!e.target.closest('.suggestion-popover')) closePopover();
            });
